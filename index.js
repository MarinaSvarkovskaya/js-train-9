...
function findElementGreaterThan(arr, num) {
  ...
  let foundElement = arr.find((index) => index > num);
  return foundElement;
}
...
function findIndexByProperty(arr, name) {
  ...
  let s = name;
  let foundIndex = arr.findIndex(({ name }) => name === s);
  return foundIndex;
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  // Повертаємо рядок.
  let qqq = arr.slice(startIdx);
  let subArrayString = qqq.join(` `);
...
function getPositiveArrayValues(arr) {
  ...
  let positiveArrayValues = arr.values();
  for (const elem of positiveArrayValues) {
    return elem;
  }
  // let d = elem.filter((item) => item > 0);
  // return d;
}
...
function removeAndDouble(arr, startIdx) {
  ...
  let sliceArray = arr.splice(0, startIdx);
  let doubledArray = sliceArray.map((num) => num * 2);
  return doubledArray;
}
...
function sumAndPrint(arr) {
  ...
  // let g = r.forEach((e) => console.log(e));
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let sd = arr.flat(2);
  let de = sd.map((i) => i * i);
  let filteredSquaredArray = de.filter((n) => n > threshold);
...
function wordLengths(arr) {
  ...
  let lengthsArray = arr.map((i) => i.length);
  return lengthsArray;
}
...
function insertElementAt(arr, index, element) {
  ...
  let s = arr.splice(index, 0, element);
  return arr;
}
...
function expandArrayByN(arr, n) {
  ...
  //
  // let expandedArray = arr.flatMap((i) => i  n);
...
function capitalizeWords(arr) {
  ...
  let capitalizedArray = arr.map((i) => i[0].toUpperCase() + i.slice(1));
